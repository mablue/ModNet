"""
ModNet v015 — سرعت + گرادیان اصلاح‌شده
======================================
- single-process batched (از v014)
- N_STEPS = 5 (کیفیت بالاتر)
- گرادیان با lr بالاتر و T نرم‌تر
"""
import os
...
# گرادیان — اصلاح‌شده
GRAD_ITERS = 1000
GRAD_LR0 = 0.05          # ← ۱۰ برابر بیشتر
GRAD_T_START = 0.5       # ← نرم‌تر
GRAD_T_END = 0.05
GRAD_LR_DECAY = 0.7
GRAD_PATIENCE = 20
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    if verbose:
        ...
        print(f"  گرادیان: lr={GRAD_LR0}, T={GRAD_T_START}..{GRAD_T_END}, "
              f"iters={GRAD_ITERS}")
        print()
    ...
    if gradient_polish and best_net is not None:
        if verbose:
            print(f"\n  💎 Surrogate Gradient (lr={GRAD_LR0}, T={GRAD_T_START})...")
        before = best_fit
        ts = time.time()
...
def main():
    print("=" * 68)
    print("  ModNet v015 — سرعت + گرادیان اصلاح‌شده")
    print("=" * 68)
