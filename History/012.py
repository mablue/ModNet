    ...
    def gradient_train_safe(self, data_in, data_tgt, steps=N_STEPS,
        ...
        for it in range(iters):
            ...
            if verbose and it % 200 == 0:
                ...
                speed = (it + 1) / max(elapsed, 1e-6)
                print(f"    it {it:4d}  lr={lr:.6f}  T={T:.3f}  "
                      f"best={best_fit:+.5f}  "
                      f"[{speed:.0f} it/s, {elapsed:.1f}s]")
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    if verbose:
        ...
        print(f"  گزارش زمان‌بندی هر {report_every} نسل")
        print()

    # ── شمارنده‌های زمان ──
    t_eval = 0.0       # ارزیابی جمعیت
    t_birth = 0.0      # تولد
    t_prune = 0.0      # هرس
    t_repro = 0.0      # تولید نسل
    n_birth_calls = 0
    n_prune_calls = 0
    ...
    with Pool(n_proc, initializer=_init_worker,
        ...
        for gen in range(gens):
            ...
            if verbose and gen > 0 and gen % report_every == 0:
                ...
                pct_eval = 100 * t_eval / max(elapsed, 1e-9)
                pct_birth = 100 * t_birth / max(elapsed, 1e-9)
                pct_prune = 100 * t_prune / max(elapsed, 1e-9)
                pct_repro = 100 * t_repro / max(elapsed, 1e-9)
                print(f"  ⏱  gen={gen}  کل={elapsed:.1f}s  "
                      f"({per_gen*1000:.0f}ms/gen)")
                print(f"     eval={t_eval:.1f}s ({pct_eval:.0f}%)  "
                      f"birth={t_birth:.1f}s ({pct_birth:.1f}%, n={n_birth_calls})  "
                      f"prune={t_prune:.1f}s ({pct_prune:.1f}%, n={n_prune_calls})  "
                      f"repro={t_repro:.1f}s ({pct_repro:.1f}%)")
    ...
    if gradient_polish and best_net is not None:
        if verbose:
            print(f"\n  💎 Surrogate Gradient...")
            print(f"  الگو: {best_net.total} نورون")
        before = best_fit
        ts = time.time()
        ...
        if verbose:
            if after > before:
                ...
                print(f"  ⚠ گرادیان سودی نداشت   {before:+.5f}  ({t_grad:.1f}s)")
            else:
                print(f"  ❌ بدتر شد   {before:+.5f} → {after:+.5f}  ({t_grad:.1f}s)")

    # ── گزارش نهایی ──
    if verbose:
        print(f"\n  ═══ گزارش زمان‌بندی نهایی ═══")
        print(f"  کل evolution: {total_time:.1f}s")
        print(f"    eval:  {t_eval:.1f}s  ({100*t_eval/total_time:.0f}%)")
        print(f"    birth: {t_birth:.1f}s  ({100*t_birth/total_time:.1f}%)  "
              f"{n_birth_calls} فراخوانی")
        print(f"    prune: {t_prune:.1f}s  ({100*t_prune/total_time:.1f}%)  "
              f"{n_prune_calls} فراخوانی")
        print(f"    repro: {t_repro:.1f}s  ({100*t_repro/total_time:.1f}%)")
        print(f"    other: {total_time - t_eval - t_birth - t_prune - t_repro:.1f}s")
...
def main():
    ...
    print(f"\n  نتیجه: fit={fit:+.5f}  زمان کل={dt:.1f}s")
