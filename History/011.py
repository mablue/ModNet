"""
ModNet v011 — Surrogate Gradient
================================
Forward: step (همون که fitness می‌بینه)
Backward: مشتق sigmoid (نرم، برای گرادیان)
"""
import os
...
class ModNet:
    ...
    def gradient_step(self, data_in, data_tgt, lr, T, steps=N_STEPS):
        """
        Forward: step(z) — همون که fitness می‌بینه
        Backward: sigmoid'(z/T) — فقط برای گرادیان
        """
        B = data_in.shape[0]
        const = data_in @ self.Win.T + self.bias
        Wt = self.W.T

        # forward با step، z رو ذخیره کن
        ...
        for it in range(iters):
            ...
            if fit > best_fit + 1e-7:
                ...
                if bad_streak >= GRAD_PATIENCE:
                    ...
                    bad_streak = 0
                    if verbose:
                        print(f"    it {it:4d}  lr↓ {lr:.6f}  T={T:.3f}  best={best_fit:+.5f}")
                    if lr < 1e-5:
                        break

            if verbose and it % 100 == 0:
                print(f"    it {it:4d}  lr={lr:.6f}  T={T:.3f}  "
                      f"fit={fit:+.5f}  best={best_fit:+.5f}")
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    if verbose:
        ...
        print(f"  gradient: surrogate، iters={GRAD_ITERS}, lr={GRAD_LR0}, "
              f"T={GRAD_T_START}..{GRAD_T_END}")
        print()
    ...
    if gradient_polish and best_net is not None:
        ...
        if verbose:
            if after > before:
                ...
                print(f"  ❌ بدتر شد   {before:+.5f} → {after:+.5f}")
...
def main():
    print("=" * 68)
    print("  ModNet v011 — Surrogate Gradient")
    print("=" * 68)
