...
ModNet — نسخه نهایی
====================
- شبکه پیمانه‌ای خودرشد بدون لایه
- بازگشتی (RNN-like)
- با هرس و تولد
- cache برای سرعت
- single-process batched eval
====================================================
نتیجه: fit=-0.00155  RMSE=0.0394  53s  20 نورون
"""
import os
...
def evolve(data_in, data_tgt, pop_size=300, gens=2000, verbose=True,
    ...
    if verbose:
        print(f"  حالت: single-process batched + caching")
        print(f"  جمعیت: {pop_size}   نسل: {gens}   N_STEPS: {N_STEPS}")
        print(f"  پیمانه‌ها: {MODULE_SIZES}   اعضا: {sum(MODULE_SIZES)}")
        print()
...
def main():
    print("=" * 68)
    print("  ModNet — نسخه نهایی")
    print("  شبکه پیمانه‌ای خودرشد، بازگشتی، بدون لایه")
    print("=" * 68)
    ...
    print(f"  مسئله: y = sin(2πx) نرمال‌شده")
    ...
    print(f"  زمان:           {dt:.1f}s")
    print(f"  پیمانه‌ها:       {net.mods}")
    print(f"  کل نورون‌ها:     {net.total}")
    print(f"  یال‌های بین‌پیمانه: {c}")
    print(f"  حلقه‌ها:         {r}")
    print(f"  یال فعال:       {W_nz}/{W_tot}")

    print(f"\n  {'x':>6s}  {'هدف':>7s}  {'پیش‌بینی':>10s}  {'خطا':>7s}")
    print(f"  {'-'*6}  {'-'*7}  {'-'*10}  {'-'*7}")
    for i in range(N):
