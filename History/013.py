"""
ModNet v013 — Batched Eval (سرعت)
Forward همه شبکه‌های هم‌اندازه، در یک einsum
"""
import os
...
# ★★★ اندازه دسته ★★★
BATCH_FACTOR = 4   # n_batches = n_proc * BATCH_FACTOR
...
def _batch_worker(nets):
    """یه لیست از شبکه، همه با هم (گروه‌بندی بر اساس اندازه)"""
    if not nets:
        return []
    results = [0.0] * len(nets)
    by_size = {}
    ...
    for size, indices in by_size.items():
        G = len(indices)
        N = size
        W_s = np.stack([nets[i].W for i in indices])       # (G, N, N)
        Win_s = np.stack([nets[i].Win for i in indices])   # (G, N, n_in)
        b_s = np.stack([nets[i].bias for i in indices])    # (G, N)
        oi_s = np.stack([nets[i].out_idx for i in indices])# (G, out_size)

        # const[g, b, n] = sum_i X[b, i] * Win[g, n, i] + bias[g, n]
        const = np.einsum('bi,gni->gbn', _DATA_IN, Win_s, optimize=True) + b_s[:, None, :]

        Wt = np.transpose(W_s, (0, 2, 1))                   # (G, N, N)
        state = np.zeros((G, B, N), dtype=np.float32)
        for _ in range(N_STEPS):
            z = np.einsum('gbn,gnm->gbm', state, Wt, optimize=True) + const
            state = (z >= 0).astype(np.float32)

        # جمع‌آوری خروجی‌ها: state[g, b, oi_s[g, k]]
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    n_batches = max(1, n_proc * BATCH_FACTOR)
    if verbose:
        print(f"  هسته‌ها: {n_proc}   جمعیت: {pop_size}   "
              f"دسته‌ها: {n_batches} (هر دسته ~{pop_size//n_batches} شبکه)")
        print()
    ...
    with Pool(n_proc, initializer=_init_worker,
        ...
        for gen in range(gens):
            # ── ارزیابی دسته‌ای ──
            ts = time.time()
            batch_size = max(1, pop_size // n_batches)
            batches = [pop[i*batch_size:(i+1)*batch_size]
                       for i in range(n_batches)]
            # آخرین دسته، باقی‌مانده رو می‌گیره
            consumed = n_batches * batch_size
            if consumed < pop_size:
                batches[-1] = batches[-1] + pop[consumed:]

            results = pool.map(_batch_worker, batches, chunksize=1)
            fits = [f for r in results for f in r]
            t_eval += time.time() - ts
...
def main():
    print("=" * 68)
    print("  ModNet v013 — Batched Eval")
    print("=" * 68)
