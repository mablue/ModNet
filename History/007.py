"""
ModNet v007 — رفع باگ‌های v006
================================
- OMP_NUM_THREADS=1 (رفع جنگ رشته‌ای)
- حذف جریمه پیری از انتخاب
- حذف pop.index (O(n²))
- crossover فقط وقتی اندازه‌ها یکی باشن
- جهش تطبیقی ساده و کنترل‌شده
"""
import os
...
class ModNet:
    ...
    def crossover(self, other):
        """فقط اگه ساختار یکسان باشه. وگرنه None."""
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True):
    ...
    if verbose:
        ...
        print(f"  OMP_NUM_THREADS=1   crossover={CROSSOVER_RATE}   "
              f"sigma={SIGMA_BASE}..{SIGMA_MAX}")
        print()
    ...
    with Pool(n_proc, initializer=_init_worker,
        ...
        for gen in range(gens):
            ...
            new_pop = list(elite)   # نخبه‌ها بدون تغییر
...
def main():
    print("=" * 68)
    print("  ModNet v007 — رفع باگ‌های v006")
    print("=" * 68)
