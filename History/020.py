...
def evolve(data_in, data_tgt, pop_size=300, gens=2000, verbose=True,
    ...
    fits_cache = [None] * pop_size   # ★ cache

    if verbose:
        print(f"  حالت: caching-eval, N_STEPS={N_STEPS}")
    ...
    for gen in range(gens):
        # ── ارزیابی فقط برای اون‌هایی که cache ندارن ──
        ts = time.time()
        ...
        while len(new_pop) < pop_size:
            ...
            new_cache.append(None)   # ★ فرزند جدید، باید eval بشه
        pop = new_pop
...
def main():
    print("=" * 68)
    print("  ModNet v020 — سرعت با caching")
    print("=" * 68)
    N = 32
