"""
Comprehensive experiment framework for ModNet (fixed)
=====================================================
...
class ModNet:
    ...
    def forward_batch(self, X, steps):
        """Return raw output bits, shape (B, out_size)."""
        B = X.shape[0]
    ...
    def predict_values(self, X, steps, POWERS, DENOM):
        """Return scalar predictions, shape (B,)."""
        out = self.forward_batch(X, steps)
        # out: (B, out_size). POWERS: (out_size,).
    ...
    def fitness(self, X, Y, steps, POWERS, DENOM, kind):
        """kind: 'boolean' or 'regression'. Y: (N,) values."""
        preds = self.predict_values(X, steps, POWERS, DENOM)  # (B,)
...
#                     Evolution
# ============================================================
def evolve(problem, max_gens, pop_size=200, verbose=True):
    X = problem['inputs']
    Y = problem['targets']          # (N,) values
    steps = problem['steps']
    n_in = problem['n_in']
    n_out = problem['n_out']
...
def make_xor2():
    ...
    Y = np.array([a ^ b for a, b in data], dtype=np.float32)  # (N,)
...
def draw_net_svg(net, path, title='', fit=None):
    n_mods = len(net.mods)
    max_size = max(net.mods)
    CS = 180
    R = 12
    PAD = 60
    TOP = 50
    W = n_mods * CS + 2 * PAD
    H = max(400, max_size * 30 + 2 * PAD + TOP)
    pos = {}
    ...
    s.append(f'<text x="{PAD}" y="28" font-size="16" fill="black">{title}</text>')
    if fit is not None:
        s.append(f'<text x="{PAD}" y="44" font-size="12" fill="#666">'
    ...
    for m in range(n_mods):
        ...
        s.append(f'<text x="{x0+8}" y="{y0+15}" font-size="10" '
                 f'fill="{col}">{lbl}</text>')
    ...
    for i in range(net.total):
        for j in range(net.total):
            ...
            s.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" '
                     f'stroke="{col}" stroke-width="{wid}" opacity="{op:.2f}"/>')
    ...
    for i in range(net.total):
        for k in range(net.n_in):
            ...
            x, y = pos[i]
            sx = x - CS // 2 + 5 + k * 6
            s.append(f'<line x1="{sx}" y1="{y-30}" x2="{x}" y2="{y}" '
                     f'stroke="#0a3" stroke-width="0.6" opacity="0.5"/>')
...
def draw_curve_svg(history, path, title=''):
    ...
    if not gens:
        return
    gmin, gmax = min(gens), max(gens) if max(gens) > 0 else 1
    ...
    def px(g):
        return PAD + (g - gmin) / max(gmax - gmin, 1) * (W - 2 * PAD)

    def py(f):
        return H - PAD - (f - fmin) / (fmax - fmin) * (H - 2 * PAD)

    s = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" '
         f'height="{H}" style="background:#fafafa;font-family:monospace">']
    s.append(f'<text x="{PAD}" y="30" font-size="16" fill="black">{title}</text>')
    s.append(f'<line x1="{PAD}" y1="{H-PAD}" x2="{W-PAD}" y2="{H-PAD}" stroke="black"/>')
    s.append(f'<line x1="{PAD}" y1="{PAD}" x2="{PAD}" y2="{H-PAD}" stroke="black"/>')
    for k in range(5):
        y = PAD + k * (H - 2 * PAD) // 4
        fv = fmax - k * (fmax - fmin) / 4
        s.append(f'<line x1="{PAD}" y1="{y}" x2="{W-PAD}" y2="{y}" stroke="#eee"/>')
        s.append(f'<text x="5" y="{y+4}" font-size="10" fill="#666">{fv:+.4f}</text>')
    pts = ' '.join(f'{px(g):.1f},{py(f):.1f}' for g, f in zip(gens, fits))
    s.append(f'<polyline points="{pts}" fill="none" '
             f'stroke="#1a5276" stroke-width="2"/>')
    s.append(f'<text x="{W-150}" y="{H-20}" font-size="11" fill="#333">'
             f'generations</text>')
    s.append(f'<text x="5" y="{PAD-10}" font-size="11" fill="#333">fitness</text>')
    s.append('</svg>')
    with open(path, 'w') as f:
        f.write('\n'.join(s))
...
def samples_to_csv(net, problem, path):
    ...
    out_bits = net.forward_batch(X, steps)                # (N, out_size)
    preds = net.predict_values(X, steps, POWERS, DENOM)   # (N,)
