"""
ModNet v019 — برگشت به v014 + گرادیان sigmoid واقعی
====================================================
"""
import os
...
class ModNet:
    ...
    # ========================================================
    # ★ گرادیان sigmoid خالص ★
    # ========================================================
    def gradient_step_sigmoid(self, data_in, data_tgt, lr, T, steps=N_STEPS):
        """
        forward با sigmoid (نرم)، backward با sigmoid' (دقیق).
        شبکه‌ای که بهینه می‌شه، شبکه sigmoid است.
        در انتهای train، fitness با step چک می‌شه.
        """
        B = data_in.shape[0]
    ...
    def gradient_train_safe(self, data_in, data_tgt, steps=N_STEPS,
        ...
        for it in range(iters):
            ...
            self.gradient_step_sigmoid(data_in, data_tgt, lr, T, steps)
            ...
            if verbose and it % 50 == 0:
                ...
                print(f"    it {it:4d}  lr={lr:.5f}  T={T:.3f}  "
...
def evolve(data_in, data_tgt, pop_size=300, gens=2000, verbose=True,
           gradient_polish=False, report_every=200):
    pop = [ModNet(N_IN, MODULE_SIZES, OUT_MODULE, N_OUT) for _ in range(pop_size)]

    if verbose:
        print(f"  حالت: single-process batched, N_STEPS={N_STEPS}")
    ...
    for gen in range(gens):
        ...
        if verbose and gen > 0 and gen % report_every == 0:
            ...
            print(f"  ⏱  gen={gen}  کل={elapsed:.1f}s  "
                  ...
                  f"eval={100*t_eval/elapsed:.0f}%")

    total_time = time.time() - t_start

    if gradient_polish and best_net is not None:
        if verbose:
            print(f"\n  💎 گرادیان sigmoid (lr={GRAD_LR0}, T={GRAD_T_START}..{GRAD_T_END})...")
            print(f"  شبکه: size={best_net.total}, fit فعلی={best_fit:+.5f}")
        before = best_fit
        ts = time.time()
...
def main():
    print("=" * 68)
    print("  ModNet v019 — نسخه نهایی (بدون Lamark)")
    print("=" * 68)
    ...
    net, fit, dt = evolve(inputs, targets, gradient_polish=False)
