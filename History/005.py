"""
ModNet v005 — نورون خالص + هرس امن
==================================
هرس فقط روی کپی، فقط اگه fit بهتر شد.
"""
import numpy as np
...
class ModNet:
    ...
    def _rebuild_mod_map(self):
        ...
        for m, sz in enumerate(self.mods):
            for i in range(s, s+sz):
                self._mod_map[i] = m
            s += sz
        # out_idx: نورون‌های پیمانه خروجی
        ...
        # اگه کمتر بود، از پیمانه‌های دیگه پد کن
    ...
    # ========================================================
    #  هرس — این بار درست و امن
    # ========================================================
    def _prune_inplace(self, data_in, min_size=MIN_MODULE_SIZE):
        """هرس درجا. برمی‌گردونه: تعداد نورون حذف‌شده"""
        ...
        # منزوی: نه ورودی داره نه خروجی
        # has_in[i] = مجموع ستون i از W + مجموع سطر i از Win  ← اصلاح باگ
        in_from_W = np.abs(self.W).sum(axis=0)      # (total,) — ورودی از نورون‌های دیگه
        in_from_in = np.abs(self.Win).sum(axis=1)   # (total,) — ورودی از خارج
        out_to = np.abs(self.W).sum(axis=1)         # (total,) — خروجی به بقیه
        has_in = (in_from_W + in_from_in) > 0
        has_out = out_to > 0
        alive &= (has_in | has_out)

        # ۳. حداقل در هر پیمانه
        per_mod_keep = []
        s = 0
        for m, sz in enumerate(self.mods):
    ...
    def try_prune(self, data_in, data_tgt, steps=N_STEPS, min_size=MIN_MODULE_SIZE):
        """
        هرس امن: یه کپی هرس‌شده برمی‌گردونه اگه fit بدتر نشد.
        اگه بدتر شد، None برمی‌گردونه.
        """
...
def evolve(data_in, data_tgt, pop_size=200, gens=1000, verbose=True):
    ...
    if verbose:
        ...
        print(f"  هرس امن هر {PRUNE_EVERY} نسل   min_module={MIN_MODULE_SIZE}")
        print()
    ...
    with Pool(n_proc, initializer=_init_worker,
        ...
        for gen in range(gens):
            ...
            if gen > 0 and gen % PRUNE_EVERY == 0:
                ...
                # آپدیت best: یا بهتر، یا هم‌کیفیت ولی کوچیک‌تر
                ...
                if fits[cur_best_idx] > best_fit + 1e-5:
                    ...
                    improved = True
                elif abs(fits[cur_best_idx] - best_fit) <= 1e-5 and pop[cur_best_idx].total < best_net.total:
                    best_net = pop[cur_best_idx].clone()
                    if verbose:
                        print(f"  gen {gen:4d}   ↻ best رو به {best_net.total} نورون آپدیت کردم")

                if verbose and total_accepted > 0:
                    c, r, W_tot, W_nz = best_net.stats()
                    print(f"  gen {gen:4d}   ✂ {total_accepted} هرس، "
                          f"-{total_removed} نورون   fit={best_fit:+.5f}   "
                          f"size={best_net.total}   wires={W_nz}/{W_tot}")

            if verbose and improved:
                c, r, W_tot, W_nz = best_net.stats()
                print(f"  gen {gen:4d}   fit={best_fit:+.5f}   "
                      f"cross={c:3d} rec={r:3d}   "
                      f"wires={W_nz}/{W_tot}   size={best_net.total}")
