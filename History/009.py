"""
ModNet v009 — رشد بی‌سقف + گرادیان با annealing
================================================
- birth: نورون جدید بدون سقف
- backprop through time (BPTT)
- sigmoid + temperature annealing (step نرم)
"""
import os
...
MIN_MODULE_SIZE = 2
MAX_MODULE_SIZE = 20        # ← سقف هر پیمانه، ولی مجموع آزاد
MAX_TOTAL = 80              # ← سقف کل
...
SIGMA_GROWTH = 1.25
PLATEAU_TRIGGER = 25
BIRTH_TRIGGER = 60          # ← اگه ۶۰ نسل گیر، نورون جدید اضافه کن
FRESH_RATE = 0.05
CROSSOVER_RATE = 0.30
...
class ModNet:
    ...
    def forward_soft(self, inputs_batch, steps, T):
        """forward با sigmoid (نرم، قابل مشتق)"""
        B = inputs_batch.shape[0]
    ...
    # ---------------- تولد: نورون جدید ----------------
    def birth(self):
        """یه نورون جدید به یه پیمانه تصادفی اضافه کن (بدون سقف کل)"""
        ...
        if not candidates:
            return None
        # پیمانه‌های کوچیک‌تر شانس بیشتری دارن
    ...
    def gradient_step(self, data_in, data_tgt, lr, T, steps=N_STEPS):
        """
        یه گام گرادیان با Backprop Through Time
        T: دمای sigmoid (1.0 = نرم، 0.01 = نزدیک step)
        """
        B = data_in.shape[0]
        # forward با cache
        states = [np.zeros((B, self.total), dtype=np.float32)]
        ...
        preds = (out @ POWERS) / DENOM
        dL_dpred = 2.0 * (preds - data_tgt) / B   # (B,)

        # گرادیان نسبت به h آخر
    ...
    def gradient_train(self, data_in, data_tgt, steps=N_STEPS,
                       iters=200, lr0=0.05, T_start=1.0, T_end=0.02,
                       verbose=False):
        """
        آموزش گرادیانی با annealing دمایی.
        sigmoid → سخت‌تر و سخت‌تر → نزدیک به step
        """
        for it in range(iters):
            # دمای آنیلینگ
            frac = it / max(1, iters - 1)
            T = T_start * (1 - frac) + T_end * frac
            lr = lr0 * (1 - frac) * 0.9 + lr0 * 0.1
            self.gradient_step(data_in, data_tgt, lr, T, steps)
            if verbose and it % 20 == 0:
                # fit سخت (با step)
                fit = self.fitness(data_in, data_tgt, steps)
                print(f"    it {it:4d}  T={T:.3f}  lr={lr:.4f}  fit(step)={fit:+.5f}")
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    if verbose:
        ...
        print(f"  birth_trigger={BIRTH_TRIGGER}   max_total={MAX_TOTAL}")
        print(f"  gradient polish: {gradient_polish}")
        print()
    ...
    if gradient_polish and best_net is not None:
        if verbose:
            print(f"\n  💎 گرادیان با annealing...")
        before = best_fit
        best_net.gradient_train(data_in, data_tgt, steps=N_STEPS,
                                iters=300, lr0=0.05,
                                T_start=1.0, T_end=0.02, verbose=verbose)
        after = best_net.fitness(data_in, data_tgt, N_STEPS)
        best_fit = after
        if verbose:
            print(f"  ✨ {before:+.5f} → {after:+.5f}  "
                  f"(بهبود {abs(after-before)/abs(before)*100:.1f}%)")
...
def main():
    print("=" * 68)
    print("  ModNet v009 — رشد بی‌سقف + گرادیان با annealing")
    print("=" * 68)
