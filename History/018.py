"""
ModNet v018 — Lamarkian Sparse
================================
گرادیان فقط روی وزن‌های موجود (غیرصفر).
شبکه sparse می‌مونه.
"""
import os
...
# Lamark — تنظیم دقیق‌تر
LAMARK_EVERY = 5        # ← از ۳ به ۵
LAMARK_ELITES = 15      # ← از ۳۰ به ۱۵
LAMARK_ITERS = 5        # ← از ۸ به ۵
LAMARK_LR = 0.02
LAMARK_T = 0.6
...
class ModNet:
    ...
    # ★ گرادیان تُنُک ★
    # ========================================================
    def gradient_step_sparse(self, data_in, data_tgt, lr, T, steps=N_STEPS):
        """
        گرادیان فقط روی وزن‌های غیرصفر (ماسک).
        ساختار شبکه حفظ می‌شه.
        """
        B = data_in.shape[0]
        ...
        # ★ ماسک: فقط وزن‌های موجود ★
        mask_W = (self.W != 0).astype(np.float32)
        mask_Win = (self.Win != 0).astype(np.float32)
        dW = dW * mask_W
        dWin = dWin * mask_Win
        # bias آزاده (هر نورون یه اسکالر)
    ...
    def lamarkian_update(self, data_in, data_tgt, iters, lr, T, steps=N_STEPS):
        """
        چند گام گرادیان تُنُک.
        اگه بهتر نشد، به حالت اول برمی‌گرده.
        """
        best_net = self.clone()
        best_fit = self.fitness(data_in, data_tgt, steps)

        for _ in range(iters):
            self.gradient_step_sparse(data_in, data_tgt, lr, T, steps)
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    t_eval = 0.0; t_birth = 0.0; t_prune = 0.0; t_repro = 0.0; t_lamark = 0.0
    n_lamark_accepted = 0
    n_lamark_total = 0
    ...
    for gen in range(gens):
        ...
        if gen > 0 and gen % LAMARK_EVERY == 0:
            ...
            for k in order[:LAMARK_ELITES]:
                ...
                new_fit = net.lamarkian_update(
                    data_in, data_tgt,
                    iters=LAMARK_ITERS, lr=LAMARK_LR, T=LAMARK_T, steps=N_STEPS
                )
                n_lamark_total += 1
            ...
            if verbose and accepted > 0:
                print(f"  gen {gen:4d}   🧬 {accepted}/{LAMARK_ELITES}   "
                      f"best={best_fit:+.5f}")
        ...
        if verbose and gen > 0 and gen % report_every == 0:
            ...
            print(f"     eval={100*t_eval/elapsed:.0f}%  "
                  ...
                  f"({n_lamark_accepted}/{n_lamark_total}="
...
def main():
    print("=" * 68)
    print("  ModNet v018 — Lamarkian Sparse")
    print("=" * 68)
    N = 32
