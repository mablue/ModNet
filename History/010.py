"""
ModNet v010 — گرادیان محافظه‌کارانه + تولد هوشمند
=================================================
- گرادیان با T کوچیک ثابت (نه annealed از 1.0)
- Track best + بازگشت اگه بدتر شد
- کاهش lr اگه گیر کرد
- تولد: فقط ۱ نورون به بهترین، نه ۴۰ تا
- هرس و تولد متعادل
"""
import os
...
PLATEAU_TRIGGER = 25
BIRTH_TRIGGER = 80
BIRTH_ELITES = 3       # ← فقط ۳ بهترین
FRESH_RATE = 0.05
CROSSOVER_RATE = 0.30

# گرادیان
GRAD_ITERS = 500
GRAD_LR0 = 0.002        # ← خیلی کوچیک‌تر
GRAD_T = 0.15           # ← نزدیک به step از همون اول
GRAD_LR_DECAY = 0.7
...
class ModNet:
    ...
    def birth(self):
        """فقط ۱ نورون جدید، به یه پیمانه تصادفی"""
    ...
    def gradient_train_safe(self, data_in, data_tgt, steps=N_STEPS,
                            iters=GRAD_ITERS, lr0=GRAD_LR0, T=GRAD_T,
                            verbose=False):
        """
        گرادیان محافظه‌کارانه:
        - T ثابت و کوچیک (نزدیک step)
        - lr کوچیک + decay
        - بهترین حالت حفظ می‌شه
        - اگه ۳ بار پشت‌سرهم بدتر شد، متوقف
        """
        best_net = self.clone()
        ...
        for it in range(iters):
            ...
            if fit > best_fit + 1e-7:
                ...
                if bad_streak >= 5:
                    ...
                    bad_streak = 0
                    if verbose:
                        print(f"    it {it:4d}  lr↓ {lr:.6f}  fit(step)={best_fit:+.5f}")
                    if lr < 1e-7:
                        break

            if verbose and it % 50 == 0:
                print(f"    it {it:4d}  lr={lr:.6f}  T={T:.3f}  fit(step)={fit:+.5f}  best={best_fit:+.5f}")
...
def evolve(data_in, data_tgt, pop_size=200, gens=1500, verbose=True,
    ...
    if verbose:
        ...
        print(f"  birth: {BIRTH_ELITES} elite هر {BIRTH_TRIGGER} نسل")
        print(f"  gradient: T={GRAD_T}, lr={GRAD_LR0}, iters={GRAD_ITERS}")
        print()
    ...
    if gradient_polish and best_net is not None:
        if verbose:
            print(f"\n  💎 گرادیان محافظه‌کارانه (T={GRAD_T}, lr={GRAD_LR0})...")
        before = best_fit
        ...
        if verbose:
            if after > before:
                ...
                print(f"  ⚠ گرادیان سودی نداشت   {before:+.5f}")
            else:
                print(f"  ❌ گرادیان بدتر شد   {before:+.5f} → {after:+.5f}")
                # برگرد به قبل گرادیان
                # (ولی نمی‌تونیم چون best_net الان خودشه...)
                # در واقع gradient_train_safe بهترین رو نگه می‌داره، پس این حالت نباید پیش بیاد
...
def main():
    print("=" * 68)
    print("  ModNet v010 — گرادیان محافظه‌کارانه")
    print("=" * 68)
